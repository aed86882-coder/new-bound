% This file is forked from the source code of the MATLAB user interface of SNOPT software.
% SNOPT is designed to support sparse gradient matrices, where the positions of nonzero
% entries of the gradient matrix is given by the user in advance. However, this function
% was not implemented in its MATLAB interface, and it simply assumed that all entries are
% nonzero, which results in being extremely slow.
% This file adds this feature to snsolve(), where two sequences iGfun and jGvar are passed in
% as struct members of "options".

function [x,fval,info,output,lambda,states] = snsolve_hack(obj, x0, A, b, varargin)
  ...
  for i = 1:numel(fields),
    if (ischar(fields{i})),
      ...
      if ~strcmp(keyword,'screen') && ...
          ...
          ~strcmp(keyword,'iGfun') && ...
          ~strcmp(keyword,'jGvar'),
...
[iAfun,jAvar,Aij] = find([sparse(1+nonlin_ineq+nonlin_eq,n); A; Aeq]);
% [iGfun,jGvar,~]   = find([ones(1+nonlin_ineq+nonlin_eq,n); sparse(linear_ineq+linear_eq,n)]);
iGfun = options.iGfun;
jGvar = options.jGvar;  % structure of the problem
